...
const categoryImageMap = {
  ...
  'ENZIMA': [
    'enzima-medical.jpg',
    'categories/enzima-product.png',
    'categories/enzima-product.svg'
  ]
};
...
const products = [
  ...
  { name: 'DL RENNOVA DIAMOND INTENSE', category: 'BIOESTIMULADOR', slug: 'dl-rennova-diamond-intense' },
  ...
  { name: 'DL RENNOVA ELLEVA X 630MG', category: 'BIOESTIMULADOR', slug: 'dl-rennova-elleva-x-630mg' },
...
function generateImageMapping() {
  console.log('=== MAPEAMENTO DE IMAGENS PARA PRODUTOS VYTALLE ===\n');
...
function checkMissingImages() {
  ...
  const existingImages = fs.readdirSync(imagesDir, { recursive: true })
    .filter(file => file.endsWith('.jpg') || file.endsWith('.png') || file.endsWith('.svg'));
  
  console.log('\n=== VERIFICAÇÃO DE IMAGENS ===');
  console.log(`📁 Imagens disponíveis: ${existingImages.length}`);
  console.log('📋 Imagens encontradas:');
  existingImages.forEach(img => console.log(`  - ${img}`));
  
  const requiredImages = new Set();
  Object.values(categoryImageMap).flat().forEach(img => requiredImages.add(img));
...
if (require.main === module) {
  console.log('🚀 Iniciando organização das imagens dos produtos...\n');
  ...
  try {
    ...
    console.log('\n💡 Próximos passos:');
    console.log('  1. Revisar o arquivo product-image-mapping.json');
    console.log('  2. Atualizar a migração do banco com os caminhos corretos');
    console.log('  3. Adicionar imagens reais dos produtos quando disponíveis');
